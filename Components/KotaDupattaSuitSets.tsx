...
export default function KotaDupattaSuitSets() {
  const kotaDupattaSuits = (productsData as Product[]).filter(product => product.collection === 'Kota Dupatta Suit Sets');

  return (
    <section className="max-w-7xl mx-auto py-12 px-4">
      <h2 className="text-2xl font-bold mb-8 text-center">KOTA DUPATTA SUIT SETS</h2>
      <div className="text-center mb-8">
        <Link href="/collections/kota-dupatta-suit-sets" className="text-sm font-medium text-gray-700 hover:text-gray-900">VIEW ALL</Link>
      </div>
      <div className="flex overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-4 gap-8 pb-4">
        {kotaDupattaSuits.map(product => (
